...
#api_key <- Sys.getenv("GENESIS_API_KEY")
#gen_auth_save("genesis", use_token = TRUE) 
gen_auth_get("genesis")
```
```{r}
bip_raw <- gen_table(
  name = "81000-0001",
...
It is convenient to set a custom theme. 
...
The data sets are loaded using the `gen_table` function from the `restatis` package. The first data set contains GDP data, while the second data set contains consumer price index (CPI) data.

```{r Get Data}
# bip_raw <- gen_download(
#   code = "81000-0001",
#   database = "genesis",
#   verbose = FALSE
# )
# 
# cpi_raw <- gen_download(
#   code = "61111-0001", 
#   database = "genesis",
#   verbose = FALSE
# )
...
cpi_raw <- gen_table(name = "61111-0001", database = "genesis")
```
